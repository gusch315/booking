...
class StaffCalendar(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        start_time = datetime.datetime.combine(start_day, datetime.time(hour=7, minute=0, second=0))
...
class Booking(generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        end = datetime.datetime(year=year, month=month, day=day, hour=hour + 2)
        if Schedule.objects.filter(staff=staff, start=start).exists():
            messages.error(self.request, '入れ違いで予約がありました。別の日時を選択してください。')
        else:
...
class MyPageDayDetail(OnlyStaffMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # カレンダー表示する最初と最後の日時の間にある予約を取得する
        start_time = datetime.datetime.combine(date, datetime.time(hour=7, minute=0, second=0))
...
class MyPageSchedule(OnlyScheduleMixin, generic.UpdateView):
    model = Schedule
    fields = ('start', 'end', 'date', 'name', 'grade', 'band')
...
def my_page_holiday_add(request, pk, year, month, day, hour):
    ...
    if staff.user == request.user or request.user.is_superuser:
        ...
        Schedule.objects.create(staff=staff, start=start, end=end, name='使用禁止(システムによる追加)')
